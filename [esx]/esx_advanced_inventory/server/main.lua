...
AddEventHandler('onMySQLReady', function ()
  if Config.WeightSqlBased == true then
    MySQL.Async.fetchAll(
      ...
      function(result)
        for i=1, #result, 1 do
          arrayWeight[result[i].name] = result[i].weight
        end
...
AddEventHandler('esx:onAddInventoryItem', function(source, item, count)
  ...
  if currentInventoryWeight > Config.Limit then
   ...
   -- ESX.CreatePickup('item_standard', item.name, qty, item.label, source_)
    TriggerClientEvent('esx:showNotification', source_, 'You made ~r~fall~s~ ' .. qty .. ' ' .. item.label )
...
RegisterServerEvent('esx_advanced_inventory:getmaxinv')
AddEventHandler('esx_advanced_inventory:getmaxinv', function()
  TriggerClientEvent('es_extended:getmaxinv', -1, Config.Limit)
end)
