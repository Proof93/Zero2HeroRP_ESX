					...
					if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
					  ...
					  if locked == 1 or class == 15 or class == 16 or class == 14 then
						  ...
						  TriggerServerEvent("esx_truck_inventory:getInventory", GetVehicleNumberPlateText(vehFront))
						   ...
						   ESX.ShowNotification('This chest is ~ r ~ closed')
					  end
					end
				else
...
Citizen.CreateThread(function()
  while true do
    ...
    Wait(0)
    if IsControlPressed(0, Keys["U"]) and (GetGameTimer() - GUI.Time) > 1000 then
		if count == 0 then
			openmenuvehicle()
			count = count +1
		else
		...
		elseif data.current.type == 'cancel' then
          ...
          --fin test
            ...
            if not max then
              ...
              ESX.ShowNotification('~r~ You are carrying too much')
            end
			    else
			      ESX.ShowNotification('~r~ Invalid quantity')
			    end
